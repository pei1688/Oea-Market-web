# Product Dialog Local State Fix Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fix "Maximum update depth exceeded" by replacing global `useProductDetailStore` with local `useState` in `ProductDialogDetail`.

**Architecture:** `ProductDialogDetail` (the quick-add cart dialog) and `ProductDetail` (the main product page) both consumed the same Zustand store, causing an infinite state-update ping-pong when the dialog opened for a different product. Moving dialog selection state to local `useState` eliminates the conflict entirely — `ProductDetail` keeps the global store, `ProductDialogDetail` manages its own ephemeral state.

**Tech Stack:** React `useState`, TypeScript

**Spec:** `docs/superpowers/specs/2026-03-20-product-dialog-local-state-design.md`
...
| Action | File |
|--------|------|
| Modify | `src/modules/product/components/product-dialog/product-dialog-item.tsx` — `ProductDialogDetail` component only |

---

## Task 1: Replace global store with local state in `ProductDialogDetail`

**Files:**
- Modify: `src/modules/product/components/product-dialog/product-dialog-item.tsx`

- [ ] **Step 1: Read the current file**

Read `src/modules/product/components/product-dialog/product-dialog-item.tsx` to confirm content before editing.
...
```bash
git add src/modules/product/components/product-dialog/product-dialog-item.tsx
git commit -m "fix: use local state in ProductDialogDetail to prevent infinite update loop"
```

- [ ] **Step 5: Manual verification**

1. Start the dev server: `npm run dev`
2. Navigate to any product page that has alsolike products
3. Click the shopping cart icon on an alsolike product
4. Verify: dialog opens without error, shows the correct product's image/variants
5. Select a variant and click "加入購物車" — item should be added to cart
6. Close the dialog, click another alsolike product's cart icon — dialog should open cleanly again with default selections reset
7. Verify the main page's product detail still works normally (variant selection, add to cart)
